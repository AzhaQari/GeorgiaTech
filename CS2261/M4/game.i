...
# 1 "loseScreen.h" 1
# 22 "loseScreen.h"
extern const unsigned short loseScreenTiles[2080];
...
void updatePlayer() {
# 78 "game.c"
    if (collMapBitmap[((((player.row + player.rdel) >> 8) + player.height - 1)*(240)+(player.col))] && collMapBitmap[((((player.row + player.rdel) >> 8) + player.height - 1)*(240)+(player.col + player.width - 1))]) {
    ...
    if ((!(~(oldButtons)&((1<<6))) && (~buttons & ((1<<6)))) && !jumped && player.row > 10) {
        player.rdel -= 1500;
        jumped = 1;
    }
    player.rdel += 150;
    ...
    if ((~((*(volatile unsigned short *)0x04000130)) & ((1<<5)))) {
        if (player.col > 2 && collMapBitmap[((player.height + ((player.row) >> 8) - 1)*(240)+(player.col - player.cdel))] &&
            collMapBitmap[((((player.row) >> 8))*(240)+(player.col - player.cdel))]) {
    ...
    if((~((*(volatile unsigned short *)0x04000130)) & ((1<<4)))) {
        if (player.col <= 232 && collMapBitmap[((player.height + ((player.row) >> 8) - 1)*(240)+(player.col + player.cdel + player.width - 1))] &&
            collMapBitmap[((((player.row) >> 8))*(240)+(player.width + player.col + player.cdel - 1))]) {
...
void drawPlayer() {
 shadowOAM[0].attr0 = (0<<14) | (0xFF & ((player.row) >> 8));
 shadowOAM[0].attr1 = (0<<14) | (0x1FF & player.col);
 waitForVBlank();
}
...
void updateRona(RONA* r) {
    if (r->active) {
        ...
        if (r->col < 0 || r->col > 232) {
            r->cdel *= -1;
            r->row = ((player.row) >> 8) - 20;
        }
        if (r->row < 1 || r->row > 140) {
            r->rdel *= -1;
        }
        if (collision(r->col, r->row, r->width, r->height, player.col, ((player.row) >> 8), player.width, player.height)) {
            health -= 3;
            r->active = 0;
...
void drawRona(RONA* r) {
    ...
    if (r->active) {
        shadowOAM[1].attr0 = (0<<14) | (0xFF & r->row);
        shadowOAM[1].attr1 = (0<<14) | (0x1FF & r->col);
        shadowOAM[1].attr2 = ((0)<<12) | ((0)*32+(1));
        waitForVBlank();
    }
