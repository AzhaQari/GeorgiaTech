...
# 1 "background.h" 1
# 22 "background.h"
extern const unsigned short backgroundTiles[240];
...
void updatePlayer() {
 ...
 if ((!(~(oldButtons) & ((1 << 5))) && (~buttons & ((1 << 5))))
  ...
  && player.row + player.height + 10 < 160 - player.rdel) {
...
void drawPlayer() {
 ...
 shadowOAM[0].attr1 = (0 << 14) | (player.col);
...
void drawLog(LOG *l) {
 shadowOAM[1].attr0 = (1 << 14) | (l->row);
 shadowOAM[1].attr1 = (2 << 14) | (l->col);
 shadowOAM[1].attr2 = ((1) << 12) | ((0)*32 + (1));
