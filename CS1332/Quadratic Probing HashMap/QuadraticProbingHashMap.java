...
public class QuadraticProbingHashMap<K, V> {
    ...
    public V remove(K key) {
        ...
        for (int i = 0; i < table.length; i++) {
            if (table[num].getKey().equals(key) && table[num] != null) {
                table[num].setRemoved(true);
                v = table[num].getValue();

                size--;
                break;
            }
            num = (num + (i * i)) % table.length;
        }
    ...
    public boolean containsKey(K key) {
        ...
        if (!table[index].isRemoved() && table[index].getKey() == key) {
            return true;
        } else {
            int i = 0;
            while (i < table.length) {
                int index2 = (index + (i * i)) % table.length;
                if (table[index2].getKey() == key && table[index2] != null
                    && !table[index2].isRemoved()) {
                    return true;
                }
            }
        }
    ...
    public Set<K> keySet() {
        ...
        for (QuadraticProbingMapEntry<K, V> value: table) {
            if (value != null) {
                keys.add((K) value.getKey());
            }
        }
        return keys;
    }
    ...
    public void resizeBackingTable(int length) {
        if (length < size) {
            throw new java.lang.IllegalArgumentException("Cannot resize if new length is less than current size.");
        }

        QuadraticProbingMapEntry<K, V>[] store = table;
        table = new QuadraticProbingMapEntry[length];

        for (int i = 0; i < store.length; i++) {
            if (store[i] != null) {
                int hash = Math.abs(store[i].getKey().hashCode() % length);
                if (table[hash] == null) {
                    table[hash] = store[i];
                } else {
                    for (int h = 0; h < length; h++) {
                        int newIndex = (hash + h * h) % length;
                        if (table[newIndex] == null) {
                            table[newIndex] = store[i];
                            break;
                        }
                    }
                }
